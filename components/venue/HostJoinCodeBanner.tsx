  ...
  return (
    <section className="space-y-3" data-testid="host-join-code-banner">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        ...
        <Link href={crewHost ? `/crew/events/${event.id}` : `/app/events/${event.id}`} className="rounded-full border border-brand-black px-4 py-2 font-bold hover:border-brand-orange hover:text-brand-orange" data-testid="host-command-link">{crewHost ? "Crew deck · go live, end the show" : "Event command center"}</Link>
        {crewHost ? null : <Link href={`/app/events/${event.id}/access`} className="rounded-full border border-brand-black px-4 py-2 font-bold hover:border-brand-orange hover:text-brand-orange">Crew &amp; guest codes</Link>}
      </div>
    </section>
  );
}
