...
export async function LiveRoomChat({ eventId, roomKind, roomId, title, description }: { eventId: string; roomKind: LiveChatRoomKind; roomId: string; title: string; description: string }) {
  ...
  return (
    <aside className="flex max-h-[42rem] min-h-[24rem] flex-col rounded-3xl border border-slate-200 bg-white sm:min-h-[30rem]" aria-label={`${title} live chat`} data-testid={`${roomKind}-live-chat`} data-chat-locked={room.locked ? "true" : "false"} data-chat-slow-mode={room.slowModeSeconds}>
