  ...
  return (
    <section className="rounded-3xl border border-brand-line bg-white p-6 shadow-sm" data-testid="guest-identity-form" data-role={role} data-has-identity={existing ? "true" : "false"}>
      ...
      <form action={readOnly ? undefined : registerGuestIdentityAction} className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <label className="grid gap-1 text-sm font-semibold text-slate-700 md:col-span-3">Email<input name="email" type="email" defaultValue={existing?.email} className="rounded-xl border border-slate-300 px-3 py-2" placeholder="ada@example.com" data-testid="guest-email" /><span className="text-xs font-normal text-slate-500">How the crew reaches you about this event. Leave it out and they cannot.</span></label>
