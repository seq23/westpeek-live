 ...
 */
export function GuestIdentityForm({ eventId, role, returnTo, existing, error, compact = false }: { eventId: string; role: SpecialGuestRole; returnTo: string; existing?: SpecialGuestProfile; error?: string; compact?: boolean }) {
  ...
  return (
    <section className="rounded-3xl border border-brand-line bg-white p-6 shadow-sm" data-testid="guest-identity-form" data-role={role} data-has-identity={existing ? "true" : "false"}>
      ...
      <form action={registerGuestIdentityAction} className="mt-4 grid gap-3 md:grid-cols-3">
        ...
        <div className="md:col-span-3"><button className="rounded-full bg-brand-black px-5 py-3 text-sm font-bold text-white hover:bg-brand-orange" data-testid="guest-identity-submit">{existing ? "Save" : "Continue"}</button></div>
      </form>
    </section>
  );
}
