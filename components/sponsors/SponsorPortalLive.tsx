 ...
 */
export async function SponsorPortalLive({ eventId, surface, sponsor, saved, error }: { eventId: string; surface: SponsorSurface; sponsor?: SpecialGuestProfile; saved?: boolean; error?: string }) {
      ...
      <section className="mx-auto max-w-6xl space-y-6">
        ...
        {sponsor && (surface === "home" || surface === "setup" || surface === "booth") ? (
          <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" data-testid="sponsor-booth-editor">
            ...
            <form action={saveSponsorBoothAction} className="mt-4 grid gap-3">
              ...
              <div><button className="rounded-full bg-brand-black px-5 py-3 text-sm font-bold text-white hover:bg-brand-orange" data-testid="save-booth">Save booth</button>{booth?.published ? <a href={`/venue/${eventId}/expo`} className="ml-3 text-sm font-bold underline">See it in the Expo</a> : null}</div>
            </form>
            <GuestIdentityForm eventId={eventId} role="sponsor" returnTo={`/sponsor/events/${eventId}/booth`} existing={sponsor} compact />
          </section>
        ) : null}
