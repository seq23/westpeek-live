...
export async function SpeakerPortalHome({ eventId, speaker, error }: { eventId: string; speaker?: SpecialGuestProfile; error?: string }) {
  ...
  return (
    <div className="space-y-6">
      <GuestIdentityForm eventId={eventId} role="speaker" returnTo={`/speaker/events/${eventId}`} existing={speaker} error={error} compact={Boolean(speaker)} />
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm" data-testid="speaker-onboarding-checklist">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <Row label="Tech check" value={techCheck ? `${techCheck.status.replaceAll("_", " ")} · ${techCheck.score}/100` : "not recorded"} href={`/speaker/events/${eventId}/tech-check`} good={techCheck?.status === "ready"} />
          <Row label="Cue cards" value={deck?.approved ? `v${deck.approved.versionNumber} approved` : deck?.pending ? "waiting for producer" : "none yet"} href={`/speaker/events/${eventId}/teleprompter`} good={Boolean(deck?.approved)} />
          <Row label="Stage" value={stage ? stage.status.replaceAll("_", " ") : "backstage"} href={`/speaker/events/${eventId}/green-room`} good={stage?.status === "on_stage"} />
        </div>
      </section>
    </div>
  );
