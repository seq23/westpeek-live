...
function LockForm({ eventId, roomKind, roomId, locked, viewer }: { eventId: string; roomKind: LiveChatRoomKind; roomId: string; locked: boolean; viewer: CrewViewer }) {
  return (
    <GatedForm viewer={viewer} action="moderate_chat" formAction={lockLiveChatRoom} className="inline">
      ...
      <button className={`rounded-full px-4 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-40 ${locked ? "border border-emerald-300 text-emerald-800" : "bg-slate-950 text-white"}`} data-testid={`chat-${locked ? "unlock" : "lock"}-${roomKind}-${roomId}`}>{locked ? "Unlock chat" : "Lock chat"}</button>
    </GatedForm>
  );
}
...
export function SilenceForm({ eventId, roomKind, roomId, attendeeId, silenced, compact = false, viewer }: { eventId: string; roomKind: LiveChatRoomKind; roomId: string; attendeeId: string; silenced: boolean; compact?: boolean; viewer: CrewViewer }) {
  return (
    <GatedForm viewer={viewer} action="moderate_chat" formAction={silenceLiveChatAttendee} className="inline">
      ...
      <button className={`rounded-full border px-3 py-1 text-xs font-black disabled:cursor-not-allowed disabled:opacity-40 ${silenced ? "border-emerald-300 text-emerald-800" : "border-rose-300 text-rose-800"} ${compact ? "" : "px-4 py-2"}`} data-testid={`chat-${silenced ? "unsilence" : "silence"}-${attendeeId}`}>{silenced ? "Unsilence" : "Silence"}</button>
    </GatedForm>
  );
}
 ...
 */
export async function ChatModerationQueue({ eventId, limit = 30, compact = false, viewer: givenViewer }: { eventId: string; limit?: number; compact?: boolean; viewer?: CrewViewer }) {
  const [queue, viewer] = await Promise.all([getLiveChatModerationQueue(eventId, limit), givenViewer ? Promise.resolve(givenViewer) : getCrewViewer(eventId)]);
          ...
          <ul className="mt-2 space-y-2 text-sm">
            {queue.silencedAttendees.map((state) => (
              <li key={state.key} className="flex flex-wrap items-center justify-between gap-2">
                ...
                <SilenceForm eventId={eventId} roomKind={state.roomKind} roomId={state.roomId} attendeeId={state.attendeeId as string} silenced compact viewer={viewer} />
              </li>
            ))}
          </ul>
        </div>
          ...
          return (
            <article key={message.id} className={`rounded-2xl p-4 ${hidden ? "border border-dashed border-rose-200 bg-rose-50/60" : "bg-slate-50"}`} data-testid="chat-moderation-row" data-message-id={message.id} data-moderation-status={message.moderationStatus}>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <GatedForm viewer={viewer} action="moderate_chat" formAction={moderateLiveChatMessage} className="inline">
                  ...
                  <button className={`rounded-full border px-3 py-1 text-xs font-black disabled:cursor-not-allowed disabled:opacity-40 ${hidden ? "border-emerald-300 text-emerald-800" : "border-slate-300 text-slate-800"}`} data-testid={`chat-${hidden ? "restore" : "hide"}-${message.id}`}>{hidden ? "Restore" : "Hide"}</button>
                </GatedForm>
                {message.attendeeId ? <SilenceForm eventId={eventId} roomKind={message.roomKind} roomId={message.roomId} attendeeId={message.attendeeId} silenced={silenced} compact viewer={viewer} /> : null}
