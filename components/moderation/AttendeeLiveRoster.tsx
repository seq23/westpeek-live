...
function DecisionButton({ eventId, roomKind, roomId, attendeeId, decision, label, tone = "neutral" }: { eventId: string; roomKind: AttendeeLiveRoomKind; roomId: string; attendeeId: string; decision: AttendeeLiveDecision; label: string; tone?: "neutral" | "primary" | "danger" | "restore" }) {
  ...
  return (
    <form action={decideAttendeeLiveAccess} className="inline">
      ...
      <button className={`rounded-full px-3 py-1 text-xs font-black ${className}`} data-testid={`roster-${decision}-${attendeeId}`}>{label}</button>
    </form>
  );
}
...
function RowActions({ eventId, roomKind, roomId, row }: { eventId: string; roomKind: AttendeeLiveRoomKind; roomId: string; row: AttendeeRosterRow }) {
  const base = { eventId, roomKind, roomId, attendeeId: row.attendeeId };
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <form action={silenceLiveChatAttendee} className="inline">
        ...
        <button className={`rounded-full border px-3 py-1 text-xs font-black ${row.silenced ? "border-emerald-300 text-emerald-800" : "border-slate-300 text-slate-800"}`} data-testid={`roster-${row.silenced ? "unsilence" : "silence"}-${row.attendeeId}`}>{row.silenced ? "Unsilence" : "Silence"}</button>
      </form>
    </div>
  );
}
 ...
 */
export async function AttendeeLiveRoster({ eventId, roomKind = "main_stage", roomId = "main-stage", search = "", searchAction }: { eventId: string; roomKind?: AttendeeLiveRoomKind; roomId?: string; search?: string; searchAction: string }) {
  const roster = await getAttendeeRoster({ eventId, roomKind, roomId, search });
  return (
            ...
            {roster.pending.map((row) => (
              <li key={row.attendeeId} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-3" data-testid={`pending-request-${row.attendeeId}`}>
                ...
                <div className="flex flex-wrap gap-2">
                  <DecisionButton eventId={eventId} roomKind={roomKind} roomId={roomId} attendeeId={row.attendeeId} decision="approve_publish" label="Approve" tone="primary" />
                  <DecisionButton eventId={eventId} roomKind={roomKind} roomId={roomId} attendeeId={row.attendeeId} decision="decline" label="Decline" tone="danger" />
                </div>
              </li>
            ))}
          </ul>
          ...
          <tbody>
            {roster.rows.length ? roster.rows.map((row) => (
              <tr key={row.attendeeId} className="border-t border-slate-100 align-top" data-testid={`roster-row-${row.attendeeId}`}>
                ...
                <td className="py-3"><RowActions eventId={eventId} roomKind={roomKind} roomId={roomId} row={row} /></td>
              </tr>
