...
import { EMAIL_NOT_CAPTURED_NOTE, listContacts, listHashOnlyPeople } from "@/services/attendees/contactsService";
...
export async function ContactsAcrossEvents({ compact = false }: { compact?: boolean }) {
  ...
  // People registered before 16 Sep 2026 have only the email hash: grouped by hash across events, listed below, counted.
  const eventNames: Record<string, string> = {};
  for (const event of await listEventRecords({ includeArchived: true, includeSeed: true }).catch(() => [])) eventNames[event.id] = event.name;
  const hashOnly = await listHashOnlyPeople(eventNames).catch(() => []);
  const total = contacts.length + hashOnly.length;
  const owner = actor?.kind === "owner";
  return (
    <SectionCard title="People across events" eyebrow={`${total} ${total === 1 ? "person" : "people"}`}>
      <div data-testid="contacts-across-events" data-count={total} data-contacts={contacts.length} data-hash-only={hashOnly.length}>
        ...
        {!total ? <div className="mt-4"><EmptyState title="No one yet" body="People appear here the first time they register for any event." /></div> : owner && contacts.length ? (
        ...
        {owner && hashOnly.length ? (
          <div className="mt-6" data-testid="hash-only-people" data-count={hashOnly.length}>
            <p className="text-xs font-black uppercase tracking-[0.25em] text-slate-500">Registered before the email was kept</p>
            <p className="mt-1 text-sm text-slate-600">{hashOnly.length} {hashOnly.length === 1 ? "person" : "people"} registered before 16 Sep 2026: their address was stored only as a hash. They are grouped here by that hash across events; the moment they register again anywhere, the email fills in on every row and they move up into the list above.</p>
            <div className="mt-3 overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="text-[11px] font-black uppercase tracking-wide text-slate-500"><tr><th className="py-2 pr-3">Name</th><th className="py-2 pr-3">Email</th><th className="py-2 pr-3">Company · title</th><th className="py-2 pr-3">Events</th><th className="py-2">Last seen</th></tr></thead>
                <tbody>
                  {hashOnly.slice(0, compact ? 20 : 500).map((person) => (
                    <tr key={person.emailHash} className="border-t border-slate-100" data-testid={`hash-only-row-${person.emailHash.slice(0, 12)}`} data-events={person.events.length}>
                      ...
                      <td className="py-2 pr-3" data-testid={`hash-only-events-${person.emailHash.slice(0, 12)}`}>{person.events.length} · {person.events.map((event) => event.name).join(", ")}</td>
