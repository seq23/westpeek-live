...
export default async function EventAreaLayout({ children, params }: { children: ReactNode; params: Promise<{ eventId: string }> }) {
  ...
  return <><BuildVersionWatchdog /><BuildVersionPoller /><EventChromeStack eventId={eventId}><SafeSection label="Event command bar" compact render={() => EventCommandBar({ eventId })} /></EventChromeStack>{children}</>;
}
