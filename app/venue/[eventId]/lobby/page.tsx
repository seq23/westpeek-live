...
export default async function LobbyPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams?: Promise<{ created?: string; error?: string }> }) {
  ...
  return (
    <VenuePageShell model={model}>
      ...
      {guest?.role === "vip" ? <VipLobbyPanel eventId={resolvedParams.eventId} error={resolvedSearchParams?.error} /> : null}
