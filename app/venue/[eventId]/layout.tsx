...
import { VenueChrome } from "@/components/venue/VenueChrome";
...
export default async function EventAreaLayout({ children, params }: { children: ReactNode; params: Promise<{ eventId: string }> }) {
  ...
  //
  // It is handed to VenueChrome rather than rendered here, because the venue nav has to sit in the
  // SAME pinned container as the bar. Two bars each pinned to top-0 is what put the nav over the
  // command bar on scroll and over the stream credentials at rest.
  return (
    <>
      <VenueChrome eventId={eventId} commandBar={<SafeSection label="Event command bar" compact render={() => EventCommandBar({ eventId })} />} />
      {children}
    </>
  );
}
