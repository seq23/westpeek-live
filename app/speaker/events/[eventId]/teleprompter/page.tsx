...
export default async function SpeakerTeleprompterPage({ params, searchParams }: { params: Promise<{ eventId: string }>; searchParams?: Promise<{ submitted?: string; error?: string }> }) {
  ...
  return (
    <SpeakerPortalShell eventId={eventId} active="teleprompter" speaker={speaker} stage={stage}>
      ...
      <SpeakerTeleprompterLive eventId={eventId} initial={{ approved: deck.approved || null, pendingVersionNumber: deck.pending?.versionNumber || null, liveCue: liveCue?.text ? liveCue : null, stage: stage || BACKSTAGE }} />
      <SpeakerCuePastePanel eventId={eventId} speaker={speaker} deck={deck} submitted={query?.submitted === "1"} />
    </SpeakerPortalShell>
  );
}
